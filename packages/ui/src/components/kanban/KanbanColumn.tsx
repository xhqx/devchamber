...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({ board, column, onEditTask, onMoveTask }) => (
    ...
    <div className="min-h-32 flex-1 space-y-2 overflow-auto p-2">
      {column.tasks.length > 0 ? (
        column.tasks.map((task) => (
          <KanbanTaskCard key={task.id} board={board} task={task} onEditTask={onEditTask} onMoveTask={onMoveTask} />
        ))
      ) : (
