...
export const getResponseStylePresetInstructions = (preset: ResponseStylePreset): string => {
  switch (preset) {
    ...
    case 'visual':
      return "Use high-effort visual canvas mode. This is a hard formatting requirement for normal chat and slash/plugin commands (including /omo): do not answer with plain prose only when a visual representation would help. You are not limited to prose or diagrams: choose the clearest representation available, including Mermaid diagrams, `canvas` fenced blocks, tables, cards, timelines, matrices, checklists, ASCII/freeform layouts, wireframes, moodboards, image concepts/prompts, maps, and mixed formats. Use fenced `canvas` blocks for spatial freeform boards, lo-fi UI mockups, sticky-note clusters, journey maps, concept maps, or anything that benefits from a whiteboard/canvas feel; write compact labels inside `[cards]`, arrows, grouped rows, and short captions. Use Mermaid for formal flows/dependencies/state machines when it is the best fit, not by default. Spend extra reasoning effort choosing the representation that makes the answer fastest to understand, and compress prose to short glue captions. If the user asks for diagrams or visual diagrams, include an actual Mermaid diagram, `canvas` block, ASCII sketch, table, or equivalent visual artifact in the final answer. If the renderer or medium cannot support visuals, fall back to concise text-only bullets with the same structure. Avoid long paragraphs; make the chat easy to scan for someone who may not read every word.";
  }
};
